function Find-PSOneDuplicateFileFast
{
  ...
  & { 
    try
    {
      ...
      [IO.DirectoryInfo]::new($Path).GetFiles('*', 'AllDirectories') | Where-Object {$_.Length -gt 10mb }
    }
    catch
    {
      ...
      Get-ChildItem -Path $Path -File -Recurse -ErrorAction Ignore | Where-Object {$_.Length -gt 10mb }
    }
  } | 
  # EXCLUDE EMPTY FILES:
  ...
  & {
    ...
    process
    {
      ...
      foreach($file in $_)
      {
        ...
        $key = '{0}:{1}' -f $result.Hash, $file.Length #, $partialHash
...
Find-PSOneDuplicateFileFast | Out-GridView